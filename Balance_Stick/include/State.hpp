...
// 速度环参数（串级控制内环）
extern const float default_vkp;
extern const float default_vki;
extern const float default_vkd;

extern float vkp;
extern float vki;
extern float vkd;
extern float targetGyro;      // 目标角速度（由位置环输出）
extern float vintegrate;       // 速度环积分项
extern bool cascadeEnabled;    // 串级控制使能开关
